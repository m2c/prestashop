...
class webcash extends PaymentModule
{
	...
	public function __construct()
	{
        ...
        $this->displayName = $this->l('webcash');
        $this->description = $this->l('Accepts payments by Webcash');
	...
	{
    return Configuration::get('webcash_SANDBOX') ? 'https://webcash.com.my/wcgatewayinit.php' : 'https://webcash.com.my/wcgatewayinit.php';
	}
	...
	{
		$this->_html = '<h2>Webcash</h2>';
		if (isset($_POST['submitwebcash']))
		{
			if (empty($_POST['merchantCode']))
				$this->_postErrors[] = $this->l('Webcash <u><b>Merchant ID</b></u> is required !');
            if (empty($_POST['merchantKey']))
				$this->_postErrors[] = $this->l('Webcash <u><b>Merchant Key</b></u> is required !</br>');
	...
	public function displaywebcash()
	{
		...
		<b>'.$this->l('This module allows you to accept payments by Webcash.').'</b><br /><br />
		<br /><br /><br />';
	}
	...
	public function displayFormSettings()
	{
		...
		<fieldset class="width3">
			...
			'.$this->l('In order to use your Webcash payment module, you have to configure your webcash account. Website : https://www.webcash.com.my').'<br /><br />
	...
	public function hookPayment($params)
	{
		...
		if (!Validate::isLoadedObject($address) OR !Validate::isLoadedObject($customer) OR !Validate::isLoadedObject($currency))
			return $this->l('webcash error: (invalid address or customer)');
	...
	{
		$translations = array(
			'mc_gross' => $this->l('webcash key \'mc_gross\' not specified, can\'t control amount paid.'),
			'payment_status' => $this->l('webcash key \'payment_status\' not specified, can\'t control payment validity'),
			'payment' => $this->l('Payment: '),
			'custom' => $this->l('webcash key \'custom\' not specified, can\'t rely to cart'),
			'txn_id' => $this->l('webcash key \'txn_id\' not specified, transaction unknown'),
			'mc_currency' => $this->l('webcash key \'mc_currency\' not specified, currency unknown'),
			'cart' => $this->l('Cart not found'),
			'order' => $this->l('Order has already been placed'),
			'transaction' => $this->l('webcash Transaction ID: '),
			'verified' => $this->l('The webcash transaction could not be VERIFIED.'),
			'connect' => $this->l('Problem connecting to the webcash server.'),
